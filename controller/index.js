...
//söker när en knapp trycks i sökrutan
function onKeyUp(){
    $("#search").keyup( 
        function (){
            getData();       
...
// Sort functions
function sortDate(n) {
    var table, rows, switching, i, x, y, fulldatex, fulldatey, splitx, splity, dir, shouldSwitch, switchcount = 0;
    ...
    while (switching) {
      ...
      for (i = 1; i < (rows.length - 1); i++) {
        ...
        fulldatex = (rows[i].getElementsByTagName("TD")[n]).innerHTML;
        fulldatey = (rows[i + 1].getElementsByTagName("TD")[n]).innerHTML;


        splitx = fulldatex.split("-");
        splity = fulldatey.split("-");


        reversedx = splitx[2] + splitx[1] + splitx[0];
        reversedy = splity[2] + splity[1] + splity[0];


        x = reversedx.toString();
        y = reversedy.toString();

        //checks if there is a NaN and changes it to 0 to ensure NaN rows stays below rows with values
        if(dir == "desc"){
            if(x == "NaN"){
                x = 0;
            } if( y == "NaN"){
...
function sortMonth(n) {
    ...
    while (switching) {
        ...
        for (i = 1; i < (rows.length - 1); i++) {
            ...
            /*Get the two elements you want to compare,
            one from current row and one from the next:*/
            x = rows[i].getElementsByTagName("TD")[n].dataset.month;
            y = rows[i + 1].getElementsByTagName("TD")[n].dataset.month;
            /*check if the two rows should switch place,
            based on the direction, asc or desc:*/
            if (dir == "asc") {
                if (x > y) {
